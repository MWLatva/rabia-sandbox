...
public:
  FixedLengthStream(std::function<T(void)> generator, int length) : generator_(generator), length_(length) {}
...
public:
  ...
  WorkloadDriver(std::unique_ptr<ClientAdapter<OpType>> client,
      ...
      : client_(std::move(client)), stream_(std::move(stream)), 
        ops_("total_ops"), stopwatch_(nullptr),
  ...
  WorkloadDriverResult ToMetrics(){
  ...
  sss::Status Run() {
    ...
    ROME_INFO("Client is terminating");
    status = client_->Stop();
    stopwatch_->Stop();
    return status;
  }
