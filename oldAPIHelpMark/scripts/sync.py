...
opts, args = getopt.getopt(sys.argv[1:], "k:u:vhi")
for opt, arg in opts:
    if opt == '-h':
        usage(False)
        sys.exit()
...
# Go to current directory from wherever the script is run
...
def main_func():
    ...
    with open(file, "r") as f:
        ...
        for i, row in enumerate(data):
            ...
            tokens = row.split(",")
            if len(tokens) != 3:
                print(f"{red}[ERR] {black}Ignoring line {i}, incorrect # of values unpacked")
                continue
            ...
            if install:
                print(f"{green}[INFO] {black}Installing dependencies on {node_id}")
