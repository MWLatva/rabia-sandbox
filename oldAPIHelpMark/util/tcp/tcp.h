...
namespace tcp {
/// @brief A struct as a standard for communicating as a server-client. It has the capability to store 4 64-bit ints.
/// In practice, I've only needed one, but the extra space could come in handy.
struct message {
  union {
    struct {
      uint64_t first;
  ...
  char padding = '\0'; // a null termination byte, in case I want to create a null-terminated char*
...
/// @brief A class for acting as a server and managing sockets with a list of clients. 
/// The interface is the server can only communicate with all the clients at once (sort of like as a herd)
/// @attention The socket manager cannot retire the singular connections of an endpoint. As a consequence of this, if one endpointmanager stopped communication while another endpointmanager continued to communicate with the server, then the socket manager blocks waiting for a message from the retired instance of the endpoint.
class SocketManager {
private:
...
public:
  ...
  void send_to_all(message* send_buffer){
    for(int i = 0; i < client_sockets.size(); i++){
      if(write(client_sockets[i], send_buffer->content.data, TCP_MESSAGE_SIZE + 1) == -1){
        ROME_FATAL("Couldn't write to socket");
      }
    }
  }
  ...
  /// @param recv_buffer an array of message of size num_clients(). This function will modify this buffer. 
  void recv_from_all(message* recv_buffer){
    for(int i = 0; i < client_sockets.size(); i++){
      if(read(client_sockets[i], recv_buffer[i].content.data, TCP_MESSAGE_SIZE + 1) == -1){
        ROME_FATAL("Couldn't read from socket");
      }
    }
  }
  ...
  /// @brief Create an instance of the socket manager. Will throw an exception if cannot connect to the socket. (Not thread-safe)
  /// @param port The port to host/connect to this SocketManager on.
  /// @param expected_conn The backlog to create when listening on the socket. If this is too low, then too many requests to connect to this socket could fail.
...
/// @brief A class for acting as a client to connect to SocketManager. This class must be used with a herd-mentality --> anything one client does, all should do
/// @attention The socket manager cannot retire the singular connections of an endpoint. As a consequence of this, if one instance stopped communication while another instance continued to communicate with the server, then the socket manager blocks waiting for a message from the retired instance of the endpoint.
...
public:
  ...
  /// @brief Create an endpoint. Will connect to host by resolving hostname (for example, on cloudlab, using nodeX sufficies to identify the intended node)
