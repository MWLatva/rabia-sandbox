...
package_deps="protobuf-compiler librdmacm-dev ibverbs-utils libspdlog-dev"
...
for i in `seq 0 ${last_valid_index}`
do
    echo "screen -t node${i} ssh ${user}@${machines[$i]}.${domain} ./${exename} --node_count ${num_nodes} --node_id ${i} --thread_count 1 --server; bash" >> ${screenrc_run}
